...
class HybridRetriever:
    ...
    def _query_embedding(self, query: str) -> list[float]:
        """Use the exact document-embedding API used during ingestion.

        This avoids Chroma's implicit query embedding path producing a vector
        with a different dimensionality from the vectors stored in the PDF
        collection.
        """
        embedding_fn = getattr(self.vs, "_embedding_function", None)
        if embedding_fn is None or not hasattr(embedding_fn, "embed_documents"):
            raise RuntimeError("Vectorstore embedding function does not expose embed_documents()")
        vector = embedding_fn.embed_documents([query])[0]
        if not vector:
    ...
    def _dense_search(self, query: str, top_k: int, metadata_filter: Optional[dict]) -> list[dict]:
        ...
        for cid, text, meta, dist in zip(
            ...
            chunks.append({
                ...
                "filename": meta.get("source", meta.get("filename", "")),
        ...
        log.info("[Dense] Retrieved %d candidates", len(chunks))
        return chunks
